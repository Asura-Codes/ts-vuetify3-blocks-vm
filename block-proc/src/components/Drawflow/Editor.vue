...
import PortIn from "./PortIn.vue";
</script>

<template>
  <div class="editor-view" ref="drawflow"></div>
</template>
...
export default {
  data: () => ({
    editor: undefined as Drawflow | undefined,
    mobile_item_selec: "",
  ...
  async mounted() {
    ...
    if (this.$refs.drawflow && internalInstance) {
      this.editor = markRaw(
        new Drawflow(
          this.$refs.drawflow as HTMLElement,
          Vue,
          internalInstance.appContext.app._context
        )
      );
      this.editor.start();
      internalInstance.appContext.config.globalProperties.$df = {
        value: this.editor,
      };
